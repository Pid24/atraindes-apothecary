...
<body>
    ...
    <section class="wrapper">
        <form action="{{ route('front.search') }}" method="GET" id="searchForm" class="w-full">
            <input
                style="background-image: url('{{ asset('assets/svgs/ic-search.svg') }}'); background-size: 20px 20px; background-repeat: no-repeat; background-position: right 10px center;"
                type="text" name="keyword" id="searchProduct"
                class="block w-full py-3.5 pl-4 pr-10 rounded-full font-semibold placeholder:text-grey placeholder:font-normal text-black text-base focus:ring-2 focus:ring-primary focus:outline-none focus:border-none transition-all"
